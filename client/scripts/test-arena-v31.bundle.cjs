...
function topDrives(typeId) {
  const def = getPersonaDrives(typeId);
  return DRIVE_KEYS.map((key) => ({ key, label: DRIVE_LABELS[key], value: def.drives[key] })).sort((a, b) => b.value - a.value).slice(0, 2);
}
...
function isSelfDisclosure(text) {
...
function createInitialState(typeId) {
  const def = getPersonaDrives(typeId);
  ...
  return {
    ...
    // 温和开局，不卑不亢
    arousal: 0.4,
    energy: isE ? 0.78 : 0.68,
    // 外向型能量满格开场
    intimacy: isF ? 0.28 : 0.2,
    // 情感型更容易打开心扉
    novelty: isP ? 0.7 : 0.55,
    // 感知型对新鲜话题更兴奋
    turnCount: 0,
    lastIntent: void 0,
    lastTopic: void 0,
...
function updateState(prev, signal) {
  ...
  switch (intent) {
    ...
    case "D": {
      ...
      next.energy = clamp(prev.energy - 0.06 * (0.4 + empathy), 0, 1);
  ...
  next.arousal = clamp(next.arousal + (0.45 - next.arousal) * 0.08, 0, 1);
...
var STATE_PREFIX = "ds_persona_state_";
function loadPersonaState(typeId) {
  try {
...
function getOrInitState(typeId) {
...
function createEmptyMemory(typeId) {
  return { typeId, entries: [], summary: "", updatedAt: Date.now() };
}
function addMemory(mem, text, kind) {
  const entry = {
...
function summarizeEntries(entries) {
  const kinds = ["fact", "preference", "event", "relationship"];
  const parts = [];
...
var MEMORY_PREFIX = "ds_persona_memory_";
function loadPersonaMemory(typeId) {
  try {
    if (typeof localStorage === "undefined") return null;
...
function getOrInitMemory(typeId) {
...
function buildDialogueSystemPrompt(typeId, opts) {
...
function parseDialogueUnderstanding(text) {
  const meta = {};
...
function stageForRound(round2, maxRounds) {
  if (round2 <= 0) return "opening";
  if (round2 === 1) return "cross_exam";
  if (round2 >= maxRounds - 1) return "summary";
  return "free_debate";
}
...
async function generateArenaSpeech(state, persona) {
...
async function runNextSpeech(state, persona) {
...
console.log("=".repeat(62));
(async () => {
  console.log("\n\u{1F3AD} 1.1 \u4EBA\u683C\u5185\u6838 personaDrives");
  const allTypeIds = mbtiProfiles.map((p) => p.id);
  check("\u8986\u76D6\u5168\u90E8 16 \u4EBA\u683C", allTypeIds.every((id) => personaDrives[id] !== void 0), `${Object.keys(personaDrives).length} \u4E2A`);
  ...
  check("\u6BCF\u4EBA\u683C\u90FD\u6709\u6E34\u671B\u4E0E\u6050\u60E7", allTypeIds.every((id) => typeof personaDrives[id]?.desire === "string" && personaDrives[id].desire.length > 0 && typeof personaDrives[id]?.fear === "string" && personaDrives[id].fear.length > 0));
  ...
  check("\u6570\u503C\u4E3A\u5343\u5206\u4F4D", heavy.intimacy === Math.round(heavy.intimacy * 1e3) / 1e3);
  ...
  console.log("\n\u{1F9E0} 3.1 \u6301\u4E45\u8BB0\u5FC6 personaMemory");
  ...
  const sum = summarizeEntries([
    { id: "1", typeId: "INTJ", text: "\u559C\u6B22\u5496\u5561", kind: "preference", ts: 1 },
    { id: "2", typeId: "INTJ", text: "\u6700\u8FD1\u5728\u5065\u8EAB", kind: "event", ts: 2 }
  ]);
...
\u3010\u56DE\u5E94\u3011\u8FD9\u662F\u4E00\u4E2A\u503C\u5F97\u6DF1\u5165\u7684\u95EE\u9898\u3002`;
  ...
  check("COT_FORMAT \u610F\u56FE\u7EF4\u5EA6\u4ECD\u5728", (buildSpeechMessages({ typeId: "INTJ", typeName: "\u5EFA\u7B51\u5E08", side: "pro", topic: TOPIC, stage: "opening", ownSpeechCount: 0, recentHistory: [] }).find((m) => m.role === "user")?.content ?? "").includes("\u5BF9\u65B9\u610F\u56FE\u7C7B\u578B"));
